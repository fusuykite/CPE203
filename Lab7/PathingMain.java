   ...
   private boolean move(Point pos, GridValues[][] grid, List<Point> path){
              ...
              grid[pos.y][pos.x] != GridValues.SEARCHED)
      {
         if (grid[pos.y][pos.x] == GridValues.GOAL){
            ...
            Point posUp = new Point(pos.x, pos.y - 1);
            Point posLeft = new Point(pos.x - 1, pos.y);
            Point posRight = new Point(pos.x + 1, pos.y);
            Point posDown = new Point(pos.x, pos.y + 1);




            valid = move(posUp, grid, path) || move(posLeft, grid, path) || move(posRight, grid, path) || move(posDown, grid, path);
         }
      }
      if (valid){
         this.path.add(pos);
